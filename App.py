...
# Load the routes from the database and create the graph
conn = sqlite3.connect('examples/example1/universe.db')
cursor = conn.cursor()
G = nx.DiGraph()
cursor.execute("SELECT ORIGIN, DESTINATION, TRAVEL_TIME FROM ROUTES")
for origin, destination, travel_time in cursor.fetchall():
...
nx.draw_networkx_edge_labels(G, pos, edge_labels={(u, v): f"{d['weight']} days" for u, v, d in G.edges(data=True)}, font_color='red')
plt.title("Universe Routes")
